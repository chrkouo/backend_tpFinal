...
import adventurers from "./domain/adventurer";
...
app.post("/signup", async(req,res)=>{
    try{
        await adventurer.createAdventurer(req.body.name, req.body.password);
        res.sendStatus(200);
    }catch(e){
        res.sendStatus(400);
    }
...
app.post("/login", async (req, res) => {
  ...
  app.get("/quests", authenticate, async (req, res) => {
    try {
      let quests = await adventurers.getAdventurerQuestsById(
        req.userToken.id
      );
      res.send(quests);
    } catch (e) {
  ...
  app.get("/me", authenticate, async (req, res) => {
    try {
      let me = await adventurers.getAdventurerById(
        req.userToken.id
      );
      res.send(me);
    } catch (e) {
